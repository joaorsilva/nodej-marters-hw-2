...
class OrdersService {
    ...
    create(token,data,callback) 
    {
        ...
        data.paymentToken = 
            ...
            && data.paymentToken.trim().length > 0
            ? data.paymentToken.trim()
            : false;
            
        if(data.paymentToken === false) {
            callback({"code": 400, "message": "Bad request", "data": {"field": "paymentToken","message": "invalid"} });
            return;
        }
                                                        ...
                                                        if(!err && data) {
                                                            ...
                                                            _this._model.create(order,function(err,orderData){
                                                                if(!err && orderData) {
                                                                    ...
                                                                    userData.orders.push(orderData.data.id)
